...
/*---------------------------------------------------------------
 ...
 USI_TWI_Master.h
---------------------------------------------------------------*/
unsigned char USI_TWI_Start_Transceiver_With_Data( unsigned char *msg, unsigned char msgSize, unsigned char releaseBus)
{
...
/*
write data using I2C protocol 
addr - destination slave address
msg - message buffer
len - length of message to write
releaseBus - flag that determines whether to release I2C bus after end of message
*/
unsigned char I2C_write(unsigned char addr, unsigned char* msg, unsigned char len, unsigned char releaseBus){
	
	unsigned char buff[len+1];
	buff[0] = (addr<<TWI_ADR_BITS) | (FALSE<<TWI_READ_BIT);
	for(unsigned char i=0;i<len;i++){
		buff[i+1] = msg[i];
	}
	return USI_TWI_Start_Transceiver_With_Data( buff, len+1, releaseBus);
}

/*
read data using I2C protocol
addr - slave address of device from which to request data
msg - message buffer to store read bytes
len - number of bytes to read
releaseBus - flag that determines whether to release I2C bus after end of message
*/
unsigned char I2C_read(unsigned char addr, unsigned char* msg, unsigned char len, unsigned char releaseBus){
	
	unsigned char buff[len+1];
	unsigned char status;
	buff[0] = (addr<<TWI_ADR_BITS) | (TRUE<<TWI_READ_BIT);
	status = USI_TWI_Start_Transceiver_With_Data( buff, len+1, releaseBus);
	//message contents start from second byte
	for(unsigned char i=0;i<len;i++){
		msg[i] = buff[i+1];
	}
	return status;
